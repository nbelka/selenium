...
public class test6 {
    ...
    public void countries(){
        ...
        /*
            В данном кейсе проверяется сортировка в справочеике countries. Для этого на вкладке countries находятся такие строки, у которых Zones > 0, открываются страницы таких строкк,
            название зон каждой страны, копируется в 2 списка, первый хранится в том порядке в котором они расположены на странице, второй потом сортируется лексографически, в конце они сравниваются.
            Если результат одинаковый, то страны раположены в правильном порядке. Данная процедура повторяется для всех стран у которых Zones > 0.
    ...
    public void geo_zones(){
        ...
        /*
            В данном кейсе проверяется сортировка в справочеике geo_zones. Для этого на вкладке geo_zones отерывается страница каждой страны, далее её зоны копируется в 2 списка,
            первый хранится в том порядке в котором они расположены на странице, второй потом сортируется лексографически, в конце они сравниваются.
            Если результат одинаковый, то зоны раположены в правильном порядке.
